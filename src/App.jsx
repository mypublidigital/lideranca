...
const supabase = createClient(
  "https://drivyvjmolrwbdccqmtp.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRyaXZ5dmptb2xyd2JkY2NxbXRwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTY2ODkxMjgsImV4cCI6MjA3MjI2NTEyOH0.B7PRyuqelHmBI5bAs8ISFA1WZcM7mCsRucZWLjdAX8M"
);

// ── Competências ──────────────────────────────────────────────────────────────
const COMPETENCIAS = [
  { key: "construirPlanos",    dbKey: "construir_planos",    label: "Construir Planos",      full: "Habilidade em construir planos" },
  { key: "resolverDesafios",   dbKey: "resolver_desafios",   label: "Resolver Desafios",     full: "Resolver desafios e desvios do plano" },
  { key: "alcanceResultados",  dbKey: "alcance_resultados",  label: "Alcance de Resultados", full: "Direcionar o time para alcance de resultados/metas" },
  { key: "criarProcessos",     dbKey: "criar_processos",     label: "Criar Processos",       full: "Criar processos" },
  { key: "desenvolverPessoas", dbKey: "desenvolver_pessoas", label: "Desenvolver Pessoas",   full: "Desenvolver pessoas" },
  { key: "desenvolverLideres", dbKey: "desenvolver_lideres", label: "Novos Líderes",         full: "Desenvolver novos líderes" },
];
...
const GLOBAL_CSS = `
  * { box-sizing: border-box; }
  .grid-id   { display: grid; grid-template-columns: 1fr 1fr 180px; gap: 16px; margin-bottom: 20px; }
  .grid-comp { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; }
  .grid-gaps { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; }
  @media (max-width: 640px) {
    .grid-id   { grid-template-columns: 1fr; }
    .grid-comp { grid-template-columns: 1fr 1fr; }
    .grid-gaps { grid-template-columns: 1fr 1fr; }
  }
  @media (max-width: 400px) {
    .grid-comp { grid-template-columns: 1fr; }
    .grid-gaps { grid-template-columns: 1fr; }
  }
`;
...
export default function App() {
  ...
  const [colaboradores,  setColaboradores]  = useState([]);   // local (formulário)
  const [dbRegistros,    setDbRegistros]    = useState([]);   // vindos do Supabase
  ...
  // ── Buscar registros do Supabase ao autenticar ────────────────────────────
  useEffect(() => {
  ...
  const buscarRegistros = async () => {
    ...
    if (!error && data) setDbRegistros(data);
    setCarregando(false);
  };

  // ── Médias baseadas nos registros do banco ────────────────────────────────
  const mediaDb = (dbKey) => {
    if (!dbRegistros.length) return 0;
    return round2(dbRegistros.reduce((s, r) => s + (Number(r[dbKey]) || 0), 0) / dbRegistros.length);
  };

  // ── Médias locais (para o formulário) ─────────────────────────────────────
  const mediaLocal = (key) => {
  ...
  const handleEnviar = async () => {
    ...
    const rows = colaboradores.map((c) => ({
      ...
      posicao:             c.posicao || "",
      nivel:               c.nivel,
      construir_planos:    c.construirPlanos,
      resolver_desafios:   c.resolverDesafios,
      alcance_resultados:  c.alcanceResultados,
      criar_processos:     c.criarProcessos,
      desenvolver_pessoas: c.desenvolverPessoas,
      desenvolver_lideres: c.desenvolverLideres,
    }));
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ background: "#2a2a2a", color: "#fff", borderRadius: 14, padding: "20px 32px", marginBottom: 24, boxShadow: "0 4px 18px rgba(0,0,0,.35)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0, marginLeft: 32 }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 800, letterSpacing: 5, color: "#fff", lineHeight: 1 }}>LEADRIX</span>
          ...
          <div onClick={() => setInstrucoes(false)}
            ...
            <div onClick={(e) => e.stopPropagation()}
              ...
              <button onClick={() => setInstrucoes(false)}
                style={{ position: "absolute", top: 16, right: 18, background: "none", border: "none", fontSize: 22, cursor: "pointer", color: "#999" }}>✕</button>
              <h2 style={{ color: "#1a237e", marginTop: 0, marginBottom: 16, fontSize: 20 }}>ℹ️ Instruções de Preenchimento</h2>
              ...
              <button onClick={() => setInstrucoes(false)} style={{ ...btnPrim, width: "100%", padding: 13, fontSize: 15 }}>
          ...
          <>
            <div style={card}>
              ...
              <div className="grid-id">
                {[{ name: "nome", label: "Nome *", placeholder: "Nome do colaborador" },
                  { name: "posicao", label: "Posição", placeholder: "Cargo / função" }].map((f) => (
                  <div key={f.name}>
                    <label style={{ display: "block", marginBottom: 6, fontWeight: 700, color: "#555", fontSize: 13 }}>{f.label}</label>
                    <input name={f.name} value={form[f.name]} onChange={handleChange} placeholder={f.placeholder} style={inp} />
                  </div>
                ))}
                <div>
              ...
              <button onClick={handleAdicionar} style={btnPrim}>+ Adicionar Colaborador</button>
            </div>
            ...
            <div style={{ ...card, overflowX: "auto" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860 }}>
                  ...
                  <tbody>
                    {colaboradores.map((c, i) => {
                      const cor = c.nivel === "Estratégico" ? { bg: "#e3f2fd", fg: "#1565c0" }
                        : c.nivel === "Operacional" ? { bg: "#fff3e0", fg: "#e65100" }
                        : { bg: "#e8f5e9", fg: "#2e7d32" };
                      return (
                        <tr key={c.id} style={{ backgroundColor: i % 2 === 0 ? "#fafafa" : "#fff" }}>
                          ...
                          <td style={{ ...tdBase, textAlign: "center" }}>
                            <span style={{ backgroundColor: cor.bg, color: cor.fg, padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{c.nivel}</span>
                          </td>
                          {COMPETENCIAS.map((comp) => (
                            <td key={comp.key} style={{ ...tdBase, textAlign: "center" }}>{c[comp.key]}</td>
                          ))}
                          <td style={{ ...tdBase, textAlign: "center", fontWeight: 700, color: "#1a237e" }}>{mediaPessoa(c)}</td>
                          <td style={{ ...tdBase, textAlign: "center" }}>
                            <button onClick={() => remover(c.id)}
                              style={{ backgroundColor: "#ffebee", color: "#c62828", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>✕</button>
                          </td>
                        </tr>
                      );
                    })}
                    <tr style={{ backgroundColor: "#e8eaf6", fontWeight: 700 }}>
                      <td colSpan={3} style={{ ...tdBase, color: "#1a237e", fontWeight: 700 }}>MÉDIA LOCAL</td>
                      {COMPETENCIAS.map((comp) => (
                        <td key={comp.key} style={{ ...tdBase, textAlign: "center", color: "#1a237e" }}>{mediaLocal(comp.key)}</td>
                      ))}
                      <td style={{ ...tdBase, textAlign: "center", color: "#1a237e" }}>
                        {round2(COMPETENCIAS.reduce((s, c) => s + mediaLocal(c.key), 0) / COMPETENCIAS.length)}
                      </td>
                      <td />
                    </tr>
                  </tbody>
              ...
              <div style={{ marginTop: 24, borderTop: "1px solid #eee", paddingTop: 20, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                <button onClick={handleEnviar} disabled={colaboradores.length === 0 || enviando}
                  style={{ backgroundColor: enviado ? "#2e7d32" : "#c62828", color: "#fff", padding: "12px 32px",
                    border: "none", borderRadius: 8, cursor: colaboradores.length === 0 ? "not-allowed" : "pointer",
                    fontWeight: 700, fontSize: 15, opacity: colaboradores.length === 0 ? .5 : 1,
                    boxShadow: "0 3px 10px rgba(0,0,0,.15)" }}>
                  {enviando ? "⏳ Enviando..." : enviado ? "✅ Enviado com sucesso!" : "📤 Enviar Resultados"}
                </button>
                {enviado && (
                  <span style={{ fontSize: 13, color: "#2e7d32", fontWeight: 600 }}>
                    Dados salvos. Acesse a aba <strong>Resultados</strong> para visualizar.
                  </span>
                )}
                {erroEnvio && (
                  <span style={{ fontSize: 13, color: "#c62828", fontWeight: 600 }}>⚠️ {erroEnvio}</span>
                )}
              </div>
            </div>
          </>
        ...
        {aba === "resultados" && !autenticado && (
          <div style={{ ...card, maxWidth: 420, margin: "60px auto", textAlign: "center" }}>
            ...
            <p style={{ color: "#777", marginBottom: 28, fontSize: 14 }}>
              Digite a senha para acessar os resultados consolidados.
            </p>
            ...
            {erroSenha && <div style={{ color: "#c62828", fontSize: 13, marginBottom: 12 }}>Senha incorreta.</div>}
            <button onClick={handleLogin} style={{ ...btnPrim, width: "100%", padding: 13, fontSize: 16 }}>Acessar</button>
          </div>
        )}
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                {carregando ? (
                  <span style={{ color: "#1a237e", fontWeight: 700, fontSize: 14 }}>⏳ Carregando dados...</span>
                ) : (
                  <span style={{ color: "#2e7d32", fontWeight: 700, fontSize: 14, backgroundColor: "#e8f5e9", padding: "8px 14px", borderRadius: 8 }}>
                    ✅ {dbRegistros.length} registro(s) no banco de dados
                  </span>
                )}
                <button onClick={buscarRegistros}
                  style={{ backgroundColor: "#e8eaf6", color: "#1a237e", border: "none", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontWeight: 700, fontSize: 13 }}>
                  🔄 Atualizar
                </button>
              </div>
            ...
            {carregando ? (
              <div style={{ ...card, textAlign: "center", padding: "48px 32px" }}>
                ...
                <p style={{ color: "#999", fontSize: 15 }}>Buscando dados do Supabase...</p>
              </div>
            ) : dbRegistros.length === 0 ? (
              <div style={{ ...card, textAlign: "center", padding: "48px 32px" }}>
                <div style={{ fontSize: 48, marginBottom: 12 }}>📭</div>
                <p style={{ color: "#999", fontSize: 15 }}>Nenhum dado enviado ainda. Compartilhe o link do formulário.</p>
              </div>
            ) : (
              <>
                {/* Tabela de todos os registros */}
                <div style={{ ...card, overflowX: "auto" }}>
                  <h2 style={{ color: "#1a237e", marginTop: 0, marginBottom: 16, fontSize: 18 }}>
                    👥 Todos os registros ({dbRegistros.length})
                  </h2>
                  <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860, fontSize: 13 }}>
                    ...
                    <tbody>
                      {dbRegistros.map((r, i) => {
                        const mediaR = round2(COMPETENCIAS.reduce((s, c) => s + (Number(r[c.dbKey]) || 0), 0) / COMPETENCIAS.length);
                        const cor = r.nivel === "Estratégico" ? { bg: "#e3f2fd", fg: "#1565c0" }
                          : r.nivel === "Operacional" ? { bg: "#fff3e0", fg: "#e65100" }
                          : { bg: "#e8f5e9", fg: "#2e7d32" };
                        return (
                          <tr key={r.id} style={{ backgroundColor: i % 2 === 0 ? "#fafafa" : "#fff" }}>
                            <td style={{ ...tdBase, fontWeight: 700 }}>{r.nome}</td>
                            <td style={{ ...tdBase, color: "#666" }}>{r.posicao || "—"}</td>
                            <td style={{ ...tdBase, textAlign: "center" }}>
                              <span style={{ backgroundColor: cor.bg, color: cor.fg, padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{r.nivel}</span>
                            </td>
                            {COMPETENCIAS.map((comp) => (
                              <td key={comp.key} style={{ ...tdBase, textAlign: "center" }}>{r[comp.dbKey]}</td>
                            ))}
                            <td style={{ ...tdBase, textAlign: "center", fontWeight: 700, color: "#1a237e" }}>{mediaR}</td>
                          </tr>
                        );
                      })}
                      <tr style={{ backgroundColor: "#e8eaf6", fontWeight: 700 }}>
                        ...
                        {COMPETENCIAS.map((comp) => (
                          <td key={comp.key} style={{ ...tdBase, textAlign: "center", color: "#1a237e" }}>{mediaDb(comp.dbKey)}</td>
                        ))}
                        <td style={{ ...tdBase, textAlign: "center", color: "#1a237e" }}>
                          {round2(COMPETENCIAS.reduce((s, c) => s + mediaDb(c.dbKey), 0) / COMPETENCIAS.length)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div style={card}>
                  <h2 style={{ color: "#1a237e", marginTop: 0, marginBottom: 20, fontSize: 18 }}>📊 Como é vs Como deveria ser</h2>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr>
                        ...
                        <th style={{ ...thBase, backgroundColor: "#283593", textAlign: "center", padding: "12px 16px" }}>
                          Como é <span style={{ fontWeight: 400, fontSize: 11 }}>(média do banco)</span>
                        </th>
                    ...
                    <tbody>
                          ...
                          <td style={{ ...tdBase, textAlign: "center" }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                              <div style={{ width: 120, height: 10, backgroundColor: "#e0e0e0", borderRadius: 6, overflow: "hidden" }}>
                                <div style={{ width: `${mediaDb(comp.dbKey) * 10}%`, height: "100%", backgroundColor: "#1a237e", borderRadius: 6 }} />
                              </div>
                              <span style={{ fontWeight: 700, color: "#1a237e", fontSize: 16, minWidth: 36 }}>{mediaDb(comp.dbKey)}</span>
                            </div>
                          </td>
                          <td style={{ ...tdBase, textAlign: "center" }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                              ...
                              <input type="number" min="0" max="10" step="0.5" value={comoDeveria[comp.key]}
                                onChange={(e) => setComoDeveria((p) => ({ ...p, [comp.key]: Number(e.target.value) }))}
                      ...
                      <tr style={{ backgroundColor: "#f3e5f5" }}>
                        ...
                        <td style={{ ...tdBase, textAlign: "center", color: "#1a237e", fontSize: 16, fontWeight: 700 }}>
                          {round2(COMPETENCIAS.reduce((s, c) => s + mediaDb(c.dbKey), 0) / COMPETENCIAS.length)}
                        </td>
                        <td style={{ ...tdBase, textAlign: "center", color: "#c62828", fontSize: 16, fontWeight: 700 }}>
                ...
                <div style={{ ...card, padding: "14px 24px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                    ...
                    {[{ id: "radar", label: "🕸️ Radar" }, { id: "linhas", label: "📈 Linhas" }, { id: "dispersao", label: "🔵 Dispersão" }].map((t) => (
                ...
                <div style={card}>
                  ...
                  <p style={{ color: "#888", fontSize: 12, marginBottom: 20 }}>
                    {tipoGrafico === "dispersao"
                      ? "Cada ponto = uma competência. Acima da diagonal = gap (deveria ser maior)."
                      : "🔵 Azul = Como é (média do banco) • 🔴 Vermelho = Como deveria ser"}
                  </p>
                  ...
                  {tipoGrafico === "radar" && (
                    <ResponsiveContainer width="100%" height={420}>
                      <RadarChart data={dadosGrafico} margin={{ top: 10, right: 60, bottom: 10, left: 60 }}>
                        ...
                        <Radar name="Como deveria ser" dataKey="Como deveria ser" stroke="#c62828" fill="#c62828" fillOpacity={0.15} strokeWidth={2.5} dot={{ r: 4 }} />
                        <Radar name="Como é" dataKey="Como é" stroke="#1a237e" fill="#1a237e" fillOpacity={0.25} strokeWidth={2.5} dot={{ r: 4 }} />
                  ...
                  {tipoGrafico === "linhas" && (
                    <ResponsiveContainer width="100%" height={400}>
                      <LineChart data={dadosGrafico} margin={{ top: 10, right: 30, bottom: 70, left: 10 }}>
                        ...
                        <Tooltip formatter={(v) => v.toFixed(2)} />
                        <Legend wrapperStyle={{ fontSize: 13 }} />
                        <Line type="monotone" dataKey="Como deveria ser" stroke="#c62828" strokeWidth={2.5} dot={{ r: 5, fill: "#c62828" }} activeDot={{ r: 7 }} />
                        <Line type="monotone" dataKey="Como é" stroke="#1a237e" strokeWidth={2.5} strokeDasharray="6 3" dot={{ r: 5, fill: "#1a237e" }} activeDot={{ r: 7 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  )}
                    ...
                    <ResponsiveContainer width="100%" height={420}>
                      <ScatterChart margin={{ top: 20, right: 30, bottom: 60, left: 20 }}>
                        ...
                        <XAxis type="number" dataKey="x" name="Como é" domain={[0, 10]} tickCount={6}
                          label={{ value: "Como é (média)", position: "insideBottom", offset: -12, fontSize: 13, fill: "#1a237e" }} tick={{ fontSize: 11 }} />
                        <YAxis type="number" dataKey="y" name="Como deveria ser" domain={[0, 10]} tickCount={6}
                          label={{ value: "Como deveria ser", angle: -90, position: "insideLeft", offset: 14, fontSize: 13, fill: "#c62828" }} tick={{ fontSize: 11 }} />
                        <ZAxis range={[90, 90]} />
                          ...
                          content={({ payload }) => {
                            ...
                            return (
                              <div style={{ backgroundColor: "#fff", border: "1px solid #ddd", borderRadius: 8, padding: "10px 14px", fontSize: 13 }}>
                                ...
                                <p style={{ margin: "2px 0 0", color: "#c62828" }}>Deveria: <strong>{d.y}</strong></p>
                                <p style={{ margin: "4px 0 0", fontWeight: 700, color: gap > 0 ? "#c62828" : "#2e7d32" }}>Gap: {gap > 0 ? `+${gap}` : gap}</p>
                              </div>
                            );
                          }} />
                ...
                <div style={card}>
                  ...
                  <div className="grid-gaps">
                    {COMPETENCIAS.map((comp) => {
                      const comoE   = mediaDb(comp.dbKey);
                      ...
                      return (
                        <div key={comp.key} style={{ backgroundColor: bg, borderRadius: 10, padding: "14px 16px", borderLeft: `4px solid ${cor}` }}>
                          ...
                          <p style={{ margin: "6px 0 0", fontWeight: 700, color: cor, fontSize: 15 }}>
                            Gap: {gap > 0 ? `+${gap}` : gap}{gap > 3 ? " 🔴" : gap > 1.5 ? " 🟡" : " 🟢"}
                          </p>
                        </div>
                      );
                    })}
